...
export default function SignatureSection() {
  ...
  const handleFilterChange = (newFilter: 'all' | 'male' | 'female') => {
    setFilter(newFilter);
                    ...
                    <div className="aspect-square relative flex-shrink-0 border-b border-border bg-[#111111]">
                        ...
                        <div className="w-full h-full flex flex-col items-center justify-center text-white">
                          <div className="w-24 h-24 border-2 border-white/20 rounded-full flex items-center justify-center">
                            <div className="text-center">
                              <span className="block font-display text-2xl font-black tracking-widest leading-none">TOPS</span>
                              <span className="block font-display text-XL font-black tracking-widest leading-none mt-1">CRE</span>
                            </div>
                          </div>
                        </div>
                      )}
                      ...
                      <div className="absolute top-4 right-4">
                        <span className={`px-3 py-1 text-[10px] font-black rounded-full uppercase tracking-tighter shadow-lg ${
                          gecko.gender === 'male' ? 'bg-blue-600 text-white' : 'bg-pink-600 text-white'
                        }`}>
                          {gecko.gender}
                        </span>
                      </div>
                    ...
                    <div className="p-6 flex flex-col flex-grow bg-card transition-colors">
                      ...
                      <div className="mt-auto">
                        <Button
                          ...
                          onClick={() => window.open(gecko.instagramUrl, '_blank', 'noopener,noreferrer')}
                        >
              ...
              {Array.from({ length: maxIndex + 1 }).map((_, index) => (
                <button
                  ...
                  className={`h-1 transition-all duration-300 ${
                    index === currentIndex ? 'bg-primary w-12' : 'bg-border w-4 hover:bg-foreground/30'
                  }`}
